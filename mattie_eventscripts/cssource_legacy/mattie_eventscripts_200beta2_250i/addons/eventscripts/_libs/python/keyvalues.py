...
class KeyValues(object):
        ...
        if existingid is not None:
            ...
            if parent is None:
                ...
                if filename is not None:
                    self.load(filename)
                if name is not None:
                    es.keypsetname(self._id_, name)
            else:
                self._parent_ = parent
                if name is None:
                    self._id_ = es.keypcreatesubkey(parent._id_)
                else:
                    self._id_ = es.keypfindsubkey(parent._id_, name)
    ...
    def __getitem__(self, name):
        ...
        '''
        x = es.keypfindsubkey(self._id_, name, False)
        if x is None or x == 0:
            ...
            if types[typ] == "TYPE_NONE":
                ...
                return es.keypgetstring(self._id_, name)
            elif types[typ] == "TYPE_INT":
                return es.keypgetint(self._id_, name)
            elif types[typ] == "TYPE_FLOAT":
                return es.keypgetfloat(self._id_, name)
##          types[4] = "TYPE_PTR"
##          types[6] = "TYPE_COLOR"
##          types[7] = "TYPE_NUMTYPES"
            else:
    ...
    def __setitem__(self, name, value):
        ...
        if (isinstance(value, KeyValues)):
            x = es.keypfindsubkey(self._id_, name, True)
            if x:
                es.keyprecursivekeycopy(x, value._id_)
        else:
            if (isinstance(value, int)):
                es.keypsetint(self._id_, name, value)
            elif (isinstance(value, float)):
                es.keypsetfloat(self._id_,name, value)
            else:
                es.keypsetstring(self._id_, name, str(value))
    def getName(self):
        return es.keypgetname(self._id_)
